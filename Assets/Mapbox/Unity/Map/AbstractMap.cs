...
namespace Mapbox.Unity.Map
{
	...
	public class AbstractMap : MonoBehaviour, IMap
	{
        ...
        private CentrarUsuario _centrarUsuario;
		...
		protected virtual void Update()
		{
            ...
            if (_camera != null && _player != null && _centrarUsuario != null && _centrarUsuario.seguir == true)
            {
                _camera.transform.position = new Vector3(_player.transform.position.x, 80, _player.transform.position.z);
            }

            if (_gps != null)
            {
                UpdateMap(new Vector2d(_gps.latitud, _gps.longitud), _zoomUsuario);
            }
            else
            {
                UpdateMap(_zoomUsuario);
            }
		...
		public virtual void UpdateMap()
		{
            ...
            {
                UpdateMap(Conversions.StringToLatLon(_options.locationOptions.latitudeLongitude), _zoomUsuario);
            }
        }
		...
		public virtual void UpdateMap(float zoom)
		{
            if(_gps != null)
            {
                UpdateMap(new Vector2d(_gps.latitud, _gps.longitud), zoom);
            }
            else
            {
        ...
        {
            //_latitud = (double)PlayerPrefs.GetFloat(latitudPrefsName, 0);
            //_longitud = (double)PlayerPrefs.GetFloat(longitudPrefsName, 0);

            _controller = GameObject.Find("Controller");

            if (_controller != null)
            {
                _gps = _controller.GetComponent<GPS>();
                if (_gps != null)
                {
                    _latitud = _gps.latitud;
                    _longitud = _gps.longitud;
                }
            }

            _botonCentrar = GameObject.Find("ButtonCenter");

            if (_botonCentrar != null)
            {
                _centrarUsuario = _botonCentrar.GetComponent<CentrarUsuario>();
            }
        ...
        protected virtual void Start()
        {
            if(_gps != null)
            {
                Initialize(new Vector2d(_gps.latitud, _gps.longitud), 15);
            }
            //MapOnStartRoutine();
        }
		...
		protected virtual void InitializeMap(MapOptions options)
		{
            ...
            if(_gps != null)
            {
                _centerLatitudeLongitude = new Vector2d(_gps.latitud, _gps.longitud);
            }
            else
            {
